...
def buildVocab(doc): 
  ...
  #  print(document)
    for word in a:
        if word != 'hhh':
            try:
                wordIndex[word][1][doc["id"]] =  (titleWeight * document[0].count(word)) / (1-.75) + .75 * len(document[0]) + (paragraphWeight * document[1].count(word)) + (dateWeight * document[2].count(word))
            except:
                wordIndex[word] = [0, {}]
